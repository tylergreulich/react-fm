...
export const getArtistInfo = (
  ...
  const response: AxiosResponse<any> | any = await axios
    .get(
      `https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=${artistName}&api_key=${process.env.REACT_APP_LAST_FM_API_KEY}&format=json`
    )
    .catch((error) => {
...
export const getArtistSimilarArtists = (artistName: string) => async (
  ...
  const response: AxiosResponse<GetSimilarArtistsData> | void = await axios
    .get(
      `https://ws.audioscrobbler.com/2.0/?method=artist.getsimilar&artist=${artistName}&limit=9&api_key=${process.env.REACT_APP_LAST_FM_API_KEY}&format=json`
    )
...
export const getArtistTopTracks = (artistName: string) => async (
  ...
  const response: AxiosResponse<TopTrackData> | void = await axios
    .get(
      `https://ws.audioscrobbler.com/2.0/?method=artist.gettoptracks&artist=${artistName}&limit=15&api_key=${process.env.REACT_APP_LAST_FM_API_KEY}&format=json`
    )
    .catch((error) => console.error(error))
...
export const getArtistsBySearch = (searchInput: string) => async (
  ...
  > | void = await axios
    .get(
      `https://ws.audioscrobbler.com/2.0/?method=artist.search&artist=${searchInput}&api_key=${process.env.REACT_APP_LAST_FM_API_KEY}&limit=10&format=json`
    )
    .catch((error) => console.error(error))
